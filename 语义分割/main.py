...
def train_(config, train_loader, net, optimizer, criterion,len_train, iter_per_epoch, writer):
    ...
    for epoch in range(config.EPOCH):  
        ...
        for i, (x,y) in enumerate(train_loader):
            ...
            pred_view = pred.view(-1)
      
            loss = criterion(pred_view, y)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
...
#             writer.add_scalar('d_loss', (d_running_loss / (i+1)), n_iter)
            ...
            if n_iter % config.SAVE_ITER == 0:
                ...
                gt = pred_image(y)
                cv2.imwrite(save_img_path+ '/' +"{}_origin_input.jpg".format(n_iter), origin)
...
def test(config, test_loader, net, criterion):
    ...
    save_origin_path = os.path.join(save_img_path, 'origin')
    if not os.path.exists(save_img_path):
        os.mkdir(save_img_path)
    ...
    for j, (x,y) in enumerate(test_loader):
        ...
        cv2.imwrite(save_origin_path+ '/' +"{}.jpg".format(j), origin)
        cv2.imwrite(save_gt_path+ '/' +"{}.jpg".format(j), gt)
        
    print("the loss is:", total_loss_test/(j+1))
    print("ending")
