...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        # Encoding path  input: [2,1,256,256]
        
        x1 = self.conv1(x)              # [2,64,256,256]
        x2 = self.conv2(self.pool(x1))  # [2,128,128,128]
        x3 = self.conv3(self.pool(x2))  # [2,256,64,64]
        x4 = self.conv4(self.pool(x3))  # [2,512,32,32]
        x5 = self.conv5(self.pool(x4))  # [2,1024,16,16]
...
class DNet(nn.Module):
    ...
    def unet_conv(self, ch_in, ch_out, is_pool= False):
        """
        Construct a convolutional unit comprising of two conv layers
    ...
    def __init__(self):
        ...
        """
        self.conv0 = self.unet_conv(3, 32, True)
        # First layer
        self.conv1 = self.unet_conv(32, 64)
        # Second layer
    ...
    def forward(self, x):
        ...
        """
        x0 = self.conv0(x)                  # [2, 32, 64, 64]
        x1 = self.conv1(self.pool(x0))      # [2, 64, 32, 32]
#         print(x1.shape)
        x2 = self.conv2(self.pool(x1))     # [2, 128, 16, 16]
#         print(x2.shape)
        x3 = self.conv3(self.pool(x2))     # [2, 256, 8, 8]
#         print(x3.shape)
        x4 = self.conv4(self.pool(x3))     # [2, 512, 4, 4]
#         print(x4.shape)
        x5 = self.conv5(self.pool(x4))     # [2, 1024, 2, 2]
#         print(x5.shape)
