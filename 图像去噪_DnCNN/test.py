...
parser.add_argument("--output", type=str, default="./checkpoints/6/set68_output", help='path of log files')
...
def main():
    ...
    for f in files_source:
        ...
        if not os.path.exists(opt.output):
            os.mkdir(opt.output)
        cv2.imwrite(opt.output+ '/' +"{}_pred.jpg".format(step), save_image(Out))
        cv2.imwrite(opt.output+ '/' +"{}_input.jpg".format(step), save_image(INoisy))
        cv2.imwrite(opt.output+ '/' +"{}_gt.jpg".format(step), save_image(ISource))
        step += 1
